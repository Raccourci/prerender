var phantom = require('phantom')
  ...
  , logAccess = bunyan.createLogger({
    name: 'phantomServerAccess',
    streams:[
      {
        level: 'log',
        path:  '/home/admin/logs/phantom/server_info.log'
      },
      {
        level: 'info',
        path:  '/home/admin/logs/phantom/server_info.log'
      ...
      {
        level: 'warn',
        path:  '/home/admin/logs/phantom/server_warn.log'
      },
      {
        level: 'error',
        path:  '/home/admin/logs/phantom/server_error.log'
      }
    ]

  })
  , logTechnical = bunyan.createLogger({
    name: 'phantomServerTechnical',
    streams:[
      {
        level: 'log',
        path:  '/home/admin/logs/phantom//technical_info.log'
      },
      {
        level: 'info',
        path:  '/home/admin/logs/phantom/technical_info.log'
      ...
      {
        level: 'warn',
        path:  '/home/admin/logs/phantom/technical_warn.log'
      },
      {
        level: 'error',
        path:  '/home/admin/logs/phantom/technical_error.log'
      }
    ]
  });
...
server.createPhantom = function () {
  var _this = this;
  logTechnical.info('starting phantom');
  ...
  var opts = {
    ...
    onExit: function () {
      _this.phantom = null;
      logTechnical.warn('phantom crashed, restarting...');
...
server.onRequest = function (req, res) {
  ...
  req.reqId = uuid.v4();
  req.log = logAccess.child({id: req.reqId, fetchedUrl: req.prerender.url});
  req.log.info('Started Request');
...
server.onResourceRequestedCallback = function (req, res, request) {
  if (!(/google-analytics.com/gi).test(request.url) && !(/api.mixpanel.com/gi).test(request.url) && !(/fonts.googleapis.com/gi).test(request.url) && !(/stats.g.doubleclick.net/gi).test(request.url) && !(/mc.yandex.ru/gi).test(request.url) && !(/use.typekit.net/gi).test(request.url) && !(/beacon.tapfiliate.com/gi).test(request.url)) {
...
server._sendResponse = function (req, res, options) {
  ...
  req.log.info({status:req.prerender.statusCode, duration: ms}, 'Page Shipped');
